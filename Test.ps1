...
function Set-CIS-2_3_SecurityOptions {
    ...
    # 2.3.15.1 System objects: Require case insensitivity for non-Windows subsystems: Enabled
    Set-CISRegValue -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Kernel' -Name 'ObCaseInsensitive' -Type DWord -Value 1 -Control "2.3.15.1"
    
    # 2.3.15.2 System objects: Strengthen default permissions of internal system objects: Enabled
    Set-CISRegValue -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager' -Name 'ProtectionMode' -Type DWord -Value 1 -Control "2.3.15.2"
    
    # 2.3.17.1 User Account Control: Admin Approval Mode for the Built-in Administrator account: Enabled
    Set-CISRegValue -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name 'FilterAdministratorToken' -Type DWord -Value 1 -Control "2.3.17.1"
    
    # 2.3.17.2 User Account Control: Behavior of the elevation prompt for administrators in Admin Approval Mode: Prompt for consent on the secure desktop
    Set-CISRegValue -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name 'ConsentPromptBehaviorAdmin' -Type DWord -Value 2 -Control "2.3.17.2"
    
    # 2.3.17.3 User Account Control: Behavior of the elevation prompt for standard users: Automatically deny elevation requests
    Set-CISRegValue -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name 'ConsentPromptBehaviorUser' -Type DWord -Value 0 -Control "2.3.17.3"
    
    # 2.3.17.4 User Account Control: Detect application installations and prompt for elevation: Enabled
    Set-CISRegValue -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name 'EnableInstallerDetection' -Type DWord -Value 1 -Control "2.3.17.4"
    
    # 2.3.17.5 User Account Control: Only elevate UIAccess applications that are installed in secure locations: Enabled
    Set-CISRegValue -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name 'EnableSecureUIAPaths' -Type DWord -Value 1 -Control "2.3.17.5"
    
    # 2.3.17.6 User Account Control: Run all administrators in Admin Approval Mode: Enabled
    Set-CISRegValue -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name 'EnableLUA' -Type DWord -Value 1 -Control "2.3.17.6"
    
    # 2.3.17.7 User Account Control: Switch to the secure desktop when prompting for elevation: Enabled
    Set-CISRegValue -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name 'PromptOnSecureDesktop' -Type DWord -Value 1 -Control "2.3.17.7"
    
    # 2.3.17.8 User Account Control: Virtualize file and registry write failures to per-user locations: Enabled
    Set-CISRegValue -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name 'EnableVirtualization' -Type DWord -Value 1 -Control "2.3.17.8"
}
...
function Set-CIS-Level2-Controls {
    ...
    # Additional Level 2 security settings
    Set-CISRegValue -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\CredentialsDelegation' -Name 'AllowProtectedCreds' -Type DWord -Value 1 -Control "18.8.4.1-L2"
...
function Set-CIS-AdditionalHardening {
    Write-Host "`n=== Additional Security Hardening ===" -ForegroundColor Cyan
    
    # Disable Windows Script Host
    Set-CISRegValue -Path 'HKLM:\SOFTWARE\Microsoft\Windows Script Host\Settings' -Name 'Enabled' -Type DWord -Value 0 -Control "Additional-WSH"
    
    # Disable Windows Installer over network
    Set-CISRegValue -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Installer' -Name 'DisableMSI' -Type DWord -Value 1 -Control "Additional-MSI"
    
    # Enable Windows Defender Real-time Protection
    try {
        Set-MpPreference -DisableRealtimeMonitoring $false -ErrorAction SilentlyContinue
        Write-Host "✓ Additional - Windows Defender real-time protection enabled" -ForegroundColor Green
        $global:AppliedControls += "Additional-Defender"
    } catch {
        Write-Verbose "Windows Defender configuration: $($_.Exception.Message)"
    }
    
    # Disable anonymous access to named pipes and shares
    Set-CISRegValue -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\LanManServer\Parameters' -Name 'RestrictNullSessAccess' -Type DWord -Value 1 -Control "Additional-NullSess"
    
    # Enable DEP for all programs
    try {
        & bcdedit.exe /set nx OptOut 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) {
            Write-Host "✓ Additional - DEP enabled for all programs" -ForegroundColor Green
            $global:AppliedControls += "Additional-DEP"
        }
    } catch {
        Write-Verbose "DEP configuration: $($_.Exception.Message)"
    }
...
function Show-Summary {
    ...
    Write-Host "8. Test printer functionality if Print Spooler was disabled" -ForegroundColor White
    Write-Host "9. Verify applications work with UAC settings" -ForegroundColor White
}
...
function Test-Prerequisites {
    ...
    $requiredExes = @('net.exe', 'auditpol.exe', 'secedit.exe', 'reg.exe', 'bcdedit.exe')
    ...
    if ($executionPolicy -eq 'Restricted') {
        Write-Warning "PowerShell execution policy is Restricted. Some features may not work properly."
    }
...
function Invoke-GroupPolicyRefresh {
    Write-Host "`n=== Refreshing Group Policy ===" -ForegroundColor Cyan
    try {
        & gpupdate.exe /force /wait:0 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) {
            Write-Host "✓ Group Policy refreshed successfully" -ForegroundColor Green
        } else {
            Write-Warning "Group Policy refresh completed with warnings"
        }
    } catch {
